...
local logfile = string.format("%s/%s.log", cacheDir, "user")
...
function Log:init()
  ...
  local log_level = Log.levels[(user.log.level):upper() or "WARN"]
  local user_log = {
    user = {
      sinks = {
  ...
  structlog.configure(user_log)
  local logger = structlog.get_logger "user"

  -- Overwrite `vim.notify` to use the logger
  if user.log.override_notify then
